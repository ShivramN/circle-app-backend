...
function authenticate (request, response, next) {
  if (request.method == 'OPTIONS') {
    ...
    __logger.info('Auth Started for ' + serviceUrl)
    ...
    try {
      ...
      if (!__util.isEmpty(token)) {
        ...
        __logger.info('decoded msg::', JSON.stringify(decoded))
        if (decoded.expire_at <= Date.now()) {
          __logger.info('Token Force Expire done, expire_at:', dateUtil.formatDate(decoded.expire_at, 'hh:mm:ss tt'))
          ...
          if (decoded.skip_inactive) {
            __logger.info('Token authorized [skip inactive]')
            next()
          } else {
            var inactiveTime = 0
            ...
            if (inactiveTime >= __config.authConfig.inactiveTimeFrame) {
              __logger.info('Token Expire due to inactive')
              __util.send(response, {
                type: __constants.RESPONSE_MESSAGES.NOT_AUTHORIZED,
                data: { message: 'Unauthorized access. Token expire due to inactive.' }
              })
            } else {
              __logger.info('Token authorized [check inactive]')
...
function generateToken (payload, expire_at, skip_inactive) {
  ...
  if (__util.isEmpty(expire_at)) {
    var time = 60 * 60
    if (__config.authConfig.forceExpireTimeFrame && typeof __config.authConfig.forceExpireTimeFrame === 'number') { time = __config.authConfig.forceExpireTimeFrame * 60 } else { __logger.info('Default 60min force expire time frame set.') }
...
//         })
//       } else {
//        __logger.info('AUTHORIZED ACCESS!! req format:: ', { remote_host: request.req_ip, uri: request.url, req_id: request.id })
//         next()
//       }
//     } catch (e) {
//       __logger.warn('UNAUTHORIZED ACCESS!! req format:: ', { remote_host: request.req_ip, uri: request.url, req_id: request.id })
