...
class MysqlLib {
  constructor () {
    __logger.info('mysqlLib.constructor called.')
    this.connections = {}
    ...
    return new Promise((resolve, reject) => {
      ...
      if (config[connName]) {
        ...
        if (!config[connName].init) {
          ...
          __logger.info('mysqlLib.init mysql[' + connName + '] not initialized.')
  ...
  query (connName, queryString, queryParam) {
    return new Promise((resolve, reject) => {
      var vm = this
      __logger.info('mysqlLib.query request::', { conn_name: connName, host: (config[connName] && config[connName].options && config[connName].options.host), query: queryString.substr(0, 50) })
      if (config[connName] && config[connName].init) {
          ...
          vm.connections[connName].conn.getConnection((err, connection) => {
            if (err) {
              ...
              const sql = connection.query(queryString, queryParam, (err, result) => {
                // __logger.info('wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww', sql.sql)
                vm.connections[connName].conn.releaseConnection(connection)
                if (err) {
                  ...
                  __logger.info('mysqlLib.query success ::', { conn_name: connName, host: (config[connName].options && config[connName].options.host) })
