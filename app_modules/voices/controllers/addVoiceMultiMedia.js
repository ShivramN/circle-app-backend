...
const upload = require('../../../lib/uploads').uploadVoices
const PlanService = require('../../subscription/services/dbData')

const addVoiceMultiMedia = (req, res) => {
  __logger.info('inside addVoiceMultiMedia :: ')
  req.bucketName = __config.voicesUrl
  const planId = req.userConfig && req.userConfig.planId ? req.userConfig.planId : 0
  const planService = new PlanService()

  planService.checkPlan(planId)
    .then((data) => {
      __logger.info('addVoiceMultiMedia :: Then 1')
      if (data && data.addVoice) {
        upload(req, res, function (err, data) {
          ...
          if (!req.file) {
            ...
            return __util.send(res, { type: __constants.RESPONSE_MESSAGES.SUCCESS, data: { voiceLoaction: { url: req.file.location, mimeType: req.file.mimetype } } })
          }
        })
      } else {
...
module.exports = addVoiceMultiMedia
