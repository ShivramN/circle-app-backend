...
const checkSuggestVoice = () => {
  return `
  SELECT v.voice_id as voiceId,v.voice_view as voiceView, v.voice_title as voiceTitle, v.voice_url as voiceUrl, ANY_VALUE(c.category_name) as categoryName,v.category_id as categoryId, v.created_on as createdOn,v.voice_platform as voicePlatform,
  IF(ANY_VALUE(sv.voice_id) IS NOT NULL, true, false) AS isSaved,
  JSON_OBJECT( 'userId', ANY_VALUE(ud.created_by),'username', ANY_VALUE(ud.username), 'fullName', ANY_VALUE(ud.full_name), 'photoUrl', ANY_VALUE(ud.photo_url), 'planName' , ANY_VALUE(s.plan_name), 'isFollow', if(ANY_VALUE(ufm.user_follow_mapping_id) IS NOT NULL , true, false), 'isCelebrity', ANY_VALUE(ud.is_celebrity)) as user,
  IF(COUNT(tv.tagged_user_id) > 0, JSON_ARRAYAGG(JSON_OBJECT('username', ud2.username, 'userId', ud2.created_by, 'fullName', ud2.full_name)), JSON_ARRAY()) AS tagUser,
  (ANY_VALUE(pv.score) / (DATEDIFF(CURDATE(), v.created_on) + 1)) AS pvScore
...
const checkFollowedVoice = () => {
  return `
  SELECT v.voice_id as voiceId,v.voice_view as voiceView, v.voice_title as voiceTitle, v.voice_url as voiceUrl,v.category_id as categoryId, ANY_VALUE(c.category_name) as categoryName, v.created_on as createdOn,v.voice_platform as voicePlatform,
  IF(ANY_VALUE(sv.voice_id) IS NOT NULL, true, false) AS isSaved,
  JSON_OBJECT( 'userId', ANY_VALUE(ud.created_by),'username', ANY_VALUE(ud.username), 'fullName', ANY_VALUE(ud.full_name), 'photoUrl', ANY_VALUE(ud.photo_url),'planName' , ANY_VALUE(s.plan_name), 'isFollow', if(ANY_VALUE(ufm.user_follow_mapping_id) IS NOT NULL , true, false), 'isCelebrity', ANY_VALUE(ud.is_celebrity)) as user,
  IF(COUNT(tv.tagged_user_id) > 0, JSON_ARRAYAGG(JSON_OBJECT('username', ud2.username, 'userId', ud2.created_by, 'fullName', ud2.full_name)), JSON_ARRAY()) AS tagUser
  FROM voices v
...
const savedVoiceList = () => {
  return `SELECT  v.voice_id as voiceId, v.voice_title as voiceTitle,v.voice_view as voiceView,v.voice_platform AS voicePlatform, v.voice_url as voiceUrl,v.category_id as categoryId,ANY_VALUE(c.category_name) as categoryName, v.created_on as createdOn,ANY_VALUE(sv.created_on) as savedOn, JSON_OBJECT( 'userId', ANY_VALUE(ud.created_by),'username', ANY_VALUE(ud.username), 'fullName', ANY_VALUE(ud.full_name), 'photoUrl', ANY_VALUE(ud.photo_url), 'isCelebrity', ANY_VALUE(ud.is_celebrity),'planName', ANY_VALUE(s.plan_name)) as user,IF(COUNT(tv.tagged_user_id) > 0,JSON_ARRAYAGG(JSON_OBJECT('username', ud2.username, 'userId', ud2.created_by, 'fullName', ud2.full_name)), JSON_ARRAY()) AS tagUser
  ...
  GROUP BY v.voice_id
  order by savedOn desc limit ? offset ?;
...
const receivedVoiceList = () => {
  return `
  SELECT  v.voice_id as voiceId, v.voice_title as voiceTitle,v.voice_view as voiceView,v.voice_platform AS voicePlatform, v.voice_url as voiceUrl,v.category_id as categoryId,ANY_VALUE(c.category_name) as categoryName, v.created_on as createdOn, ANY_VALUE(sv.created_on) as sharedOn, JSON_OBJECT( 'userId', ANY_VALUE(ud.created_by),'username', ANY_VALUE(ud.username), 'fullName', ANY_VALUE(ud.full_name), 'isCelebrity', ANY_VALUE(ud.is_celebrity), 'planName', ANY_VALUE(s.plan_name)) as user, IF(COUNT(tv.tagged_user_id) > 0,JSON_ARRAYAGG(JSON_OBJECT('username', ud2.username, 'userId', ud2.created_by, 'fullName', ud2.full_name)), JSON_ARRAY()) AS tagUser
    ...
    GROUP BY v.voice_id
    order by sharedOn desc limit ? offset ?;
...
const tagVoiceList = () => {
  return `
  SELECT  v.voice_id as voiceId, v.voice_title as voiceTitle,v.voice_view as voiceView,v.voice_platform AS voicePlatform, v.voice_url as voiceUrl,v.category_id as categoryId,ANY_VALUE(c.category_name) as categoryName, v.created_on as createdOn,ANY_VALUE(tv.created_on) as tagOn, IF(COUNT(tv.tagged_user_id) > 0,JSON_ARRAYAGG(JSON_OBJECT('username', ud2.username, 'userId', ud2.created_by, 'fullName', ud2.full_name)), JSON_ARRAY()) AS tagUser, JSON_OBJECT( 'userId', ANY_VALUE(ud.created_by),'username', ANY_VALUE(ud.username), 'fullName', ANY_VALUE(ud.full_name), 'createdOn', ANY_VALUE(tv.created_on), 'isCelebrity', ANY_VALUE(ud.is_celebrity),'planName', ANY_VALUE(s.plan_name)) as user
      FROM tag_voice tv
      ...
      GROUP BY v.voice_id
      order by tagOn desc limit ? offset ?;
